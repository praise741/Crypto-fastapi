...
class PortfolioAccount(BaseModel):
    ...
    type: Mapped[str] = mapped_column(
        Enum("CSV", name="portfolio_account_type"), nullable=False
    )
...
class PortfolioHolding(BaseModel):
    ...
    account_id: Mapped[str] = mapped_column(
        ForeignKey("portfolio_accounts.id", ondelete="CASCADE"), nullable=False
    )
    asset_symbol: Mapped[str] = mapped_column(String(64), nullable=False)
    quantity: Mapped[float] = mapped_column(
        Numeric(precision=24, scale=10), nullable=False
    )
    cost_basis: Mapped[float | None] = mapped_column(
        Numeric(precision=24, scale=10), nullable=True
    )
